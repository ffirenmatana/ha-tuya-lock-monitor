"""Binary sensors for Tuya Lock Monitor v2.

Changes vs v1:
  * `automatic_lock` is renamed to "Auto-lock Armed" to make its read-only
    nature obvious (it's `True` while the auto-lock timer is counting down
    after an unlock, and `False` once the lock has re-latched).
"""
